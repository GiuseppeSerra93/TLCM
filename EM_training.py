...
for category in categories:
    ...
    def e_step(reviews, p_zu_u, p_zp_p, p_w_yuyp, alpha_u, alpha_p, K, L, V):
        ...
        output_dict_zuzp = {i: defaultdict(dict) for i in
                            range(V)}  # {word_id:{user_id: {prod_id: P(z_uk,z_pl|w,u,p)}}}
        output_dict_yuyp = {i: defaultdict(dict) for i in
                            range(V)}  # {word_id:{user_id: {prod_id: P(y_uk',y_pl'|w,u,p)}}}
        ...
        for review in tqdm(reviews):
            ...
            for word in words:
                ...
                #     P(z_uk,z_pl|w,u,p)
                # ==============================================================================
                p_word_yuyp = p_w_yuyp_T[word].reshape(K * L,
                                                       1)  # p(word_id | y_uk', y_pl') = (K*L,) (is not a distribution)
    ...
    while i < max_iter and patience <= max_patience:
        ...
        output_dict_zuzp, output_dict_yuyp = e_step(reviews_train, p_zu_u, p_zp_p, p_w_yuyp, alpha_user, alpha_prod, K,
                                                    L, V)
